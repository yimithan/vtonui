 ...
 * pipelines are interchangeable. Every prompt and generation setting is passed
 * through untouched — only the transport differs.
 *
 *  - Analysis (vision -> text prompt):  OpenRouter on fal, OpenAI-compatible
 *    chat-completions endpoint, default model "google/gemini-3-pro".
 *  - Image generation/editing:          fal-ai/gemini-3-pro-image-preview/edit
 *    (a.k.a. Nano Banana Pro / "Gemini 3 Pro Image"), which accepts a prompt
 *    plus image_urls.
 *
 * The app's existing Gemini model identifiers are mapped to fal endpoints /
 * OpenRouter slugs below. Adjust the slugs if OpenRouter exposes the Gemini 3
 * family under different names.
 */
...
// App imageModel id -> fal image edit endpoint
...
const FAL_TEXT_FALLBACK = 'google/gemini-3-pro';
const FAL_IMAGE_FALLBACK = 'fal-ai/gemini-3-pro-image-preview/edit';
...
export const analyzeImages = async (
  ...
  plog(
    processId,
    'proof',
    `[fal:analyze] MODEL MAP PROOF — app promptModel="${promptModel}" -> fal textModel="${model}"${FAL_TEXT_MODEL_MAP[promptModel] ? '' : ' (FALLBACK — requested id not mapped)'}`,
  );
