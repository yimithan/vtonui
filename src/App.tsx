...
export default function App() {
  ...
  const handleGenerate = async () => {
    ...
    addLog('info', `[Batch] Starting AI Clothing batch — ${selectedStudioModes.length} mode(s) × ${modelImages.length} model(s) × ${validGroups.length} garment group(s) = ${totalCombinations} combination(s)`);
    ...
    const processCombination = async ({ modelIdx, group, promptMode }: { modelIdx: number; group: GarmentGroup; promptMode: PromptMode }) => {
      ...
      try {
        ...
        addLog('error', `[AI Clothing] Failed — mode "${promptMode}", model "${modelImage.file.name}", garment group ${group.id}: ${error.message || 'Unknown error'}`);
        setResults(prev => prev.map(r =>
          (r.modelId === modelId && r.garmentId === group.id && r.promptMode === promptMode)
  ...
  const handleGeneratePose = async () => {
    ...
    const processCombination = async ({ modelIdx, poseIdx, pose }: { modelIdx: number; poseIdx: number; pose: string }) => {
      ...
      try {
        ...
        addLog('error', `[Pose Generator] Failed — model "${modelImage.file.name}", pose ${poseIdx + 1}: ${error.message || 'Unknown error'}`);
        setPoseResults(prev => prev.map(r =>
          (r.modelId === modelId && r.garmentId === garmentId)
            ? { ...r, status: 'error', error: error.message || "Unknown error" }
            : r
          ...
          <main className="h-full overflow-y-auto p-8">
            <div className="max-w-5xl mx-auto space-y-8">
              <div>
                ...
                <p className="text-slate-400">Select a function to continue.</p>
              </div>
